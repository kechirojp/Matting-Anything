"""DEVA 方式 SAM2 + BEN2 / transparent-background ハイブリッド動画αマットデモ（手動プロンプト版）。

``gradio_app_sam2_ben2_tb_hybrid_deva_for_Movie`` を土台に、参考アプリ
（``gradio_app_sam2_ben2_route_a_deva_manual_for_Movie``）の **手動 box / point（positive /
negative）プロンプト canvas** を統合した版。DEVA 方式の「周期再検出 × SAM2 伝播 × consensus 統合」を
維持しつつ、先頭フレームで対象を正確に seed したり、negative point で誤検出領域を除外できる。
alpha 生成段は ``BEN2TransparentHybridVideoExtractor``（人物 mask 内は transparent-background、
外側は BEN2）をそのまま利用する。

2 つの動作モード（``DevaSemiOnlineTracker`` が引数で自動判定）:
    - モードA（手動 seed のみ）: Text Prompt を空にし、Canvas の box/point だけで追跡。
      検出島を走らせず、全フレームを単一クリップで伝播する。
...
GroundingDINO 上に再構成したものである（プロジェクト方針）。手動 seed の SAM2 伝播は既存
``DevaSemiOnlineTracker`` の initial_boxes / initial_points / initial_labels 経路
（negative=label 0 含む）をそのまま利用し、ハイブリッドパイプラインは無改変で再利用する。
"""
...
def select_sam2_prompt_multi(base_image, prompt_mode: str, point_label, prompt_state: dict | None, evt: gr.SelectData):
    """複数 box を蓄積できるよう共有 ``select_sam2_prompt`` をラップする。

    box モードで新しい box の1点目をクリックした瞬間に、直前に確定していた box を
    ``prompt_state["boxes"]``（union リスト）へコミットしてから新しい box の作図を始める。
    これにより「囲む→囲む→…」の操作だけで対象を複数追加でき、直近の box は
    ``prompt_state["box"]`` に残るため Extend（端延長）もそのまま使える。共有ヘルパー自体は
    無改変で、静止画版など他アプリの単一 box 挙動には影響しない。

    Args:
        base_image: overlay を含まないクリーンフレーム（描画基準・ERR063 対策）。
        prompt_mode: "point" または "box"。
        point_label: point モード時の positive / negative。
        prompt_state: 現在の SAM2 prompt state。
        evt: Gradio の選択イベント（クリック座標を保持）。

    Returns:
        tuple: (overlay 描画済みプレビュー, 更新後 state, ステータス文字列)。
    """
    state_before = copy_prompt_state(prompt_state)
    previous_box = state_before.get("box")
    previous_buffer = state_before.get("box_buffer") or []

    preview, state, status = select_sam2_prompt(base_image, prompt_mode, point_label, prompt_state, evt)

    # box モードで「新しい box の1点目」を打った直後（buffer 0→1）かつ直前 box が確定済みなら、
    # 直前 box を union へ退避して複数 box を蓄積する（重複は追加しない）。
    if prompt_mode == "box" and previous_box is not None:
        new_buffer = state.get("box_buffer") or []
        if len(previous_buffer) == 0 and len(new_buffer) == 1:
            union_boxes = [list(single) for single in (state.get("boxes") or [])]
            if list(previous_box) not in union_boxes:
                union_boxes.append(list(previous_box))
            state["boxes"] = union_boxes
            state["box"] = None
            preview = draw_prompt_overlay(base_image, state)
            status = (
                f"box を {len(union_boxes)} 個保持しました。新しい box の1点目を選択（対角2点で確定）。"
            )
    return preview, state, status
...
# ─────────────────────────────────────────
# ## DEVA 方式 + 手動プロンプト + BEN2/TB hybrid 実行系
# ─────────────────────────────────────────
def run_deva_hybrid_manual_background_removal(
    video_path: str | None,
    prompt_state: dict | None,
    text_prompt: str,
    detection_every: int,
    ...
    """DEVA 追跡 mask（手動 seed 併用）で、人物TB＋その他BEN2の hybrid alpha を出力する。"""
    ...
    try:
        ...
        # union（複数 box）に、まだ union へ退避されていない直近の手動 box を重複なく足す。
        # これにより Canvas で複数 box を囲んだ場合も全 box を別オブジェクトとして seed できる。
        ...
        progress(
            0.03,
            desc=(
                f"Pipeline を起動しています（モード{mode_label}）。初回はモデル読込"
        ...
        status = (
            f"完了: モード{mode_label}, output_mode={output_mode}, composition={composition_mode}, "
        ...
        raise gr.Error(f"DEVA + 手動プロンプト hybrid 動画処理に失敗しました（stage={stage}, elapsed={elapsed:.1f}s）: {exc}") from exc
...
with gr.Blocks(title="SAM2 + BEN2/TB Hybrid (DEVA 方式 + 手動プロンプト) for Movie") as demo:
    gr.Markdown("# SAM2 + BEN2/TB Hybrid（DEVA 方式 + 手動 box / point プロンプト）for Movie")
    gr.Markdown(
        """
> ## このアプリの特徴
> **BEN2 は主被写体をつかむ力**、**transparent-background は人物・髪の毛の繊細な切り抜き**
> を担当します。DEVA 方式（**周期再検出 × SAM2 伝播 × consensus 統合**）に、
...
> - 追跡した人物 mask 内は transparent-background、mask 外側は BEN2 の alpha を使い、最後に合成します。
"""
    )
    gr.Markdown(
        """
### 使い方
1. **Input Video** に動画をアップロードします（第 1 フレームが Canvas に自動表示されます）。
2. **SAM2 Prompt Canvas** で対象を box で囲むか、point（positive / negative）でヒントを与えます。
3. さらに追従を安定させたい場合は **Text Prompt** に対象を意味で入力します（モードB）。
4. **Alpha 合成方式** はまず `lighten / 比較明（推奨）` を選びます。
5. まずは既定値（最大 30 frames）で **Hybrid 実行** し、Tracking Overlay で mask を確認します。

**処理順**: `フレーム取得 → DEVA（手動 seed × 周期再検出 × SAM2 伝播 × consensus）→ 人物領域TB / 外側BEN2 → alpha合成 → 出力`。
"""
    )

    with gr.Tab("DEVA + 手動プロンプト + BEN2/TB Hybrid"):
        with gr.Row():
        ...
        with gr.Row():
            prompt_mode = gr.Radio(
                ...
                info="box: Prompt Canvas 上で対角 2 点をクリックし対象を囲む矩形を指定。続けて別の対象を囲むと box を複数追加でき、各 box を別オブジェクトとして追跡する。point: クリック1点ごとに前景/背景ヒントを与える（複数点可）。単位なしの選択値。",
            )
        ...
        # clear / 個別削除 を冪等に UI 反映させる（ERR063 系の overlay 焼き込み防止）。
        ...
        run_btn = gr.Button("DEVA + 手動 Hybrid 実行", variant="primary")
...
"""
    ...
    run_btn.click(
        run_deva_hybrid_manual_background_removal,
        inputs=[
...
def parse_args() -> argparse.Namespace:
    """CLI 引数を解析する。"""
    parser = argparse.ArgumentParser(description="SAM2 + BEN2/TB hybrid (DEVA method + manual prompt) Haystack movie demo")
    ...
    parser.add_argument("--server-port", type=int, default=7866, help="Gradio server port")
