...
| `gradio_app_sam2_ben2_route_a_for_Movie.py` | ルートA案（SAM2 伝搬 + BEN2 再α化）動画デモ（port 7862・ベース） | DEVA 版の改変ベース。**無改変を維持**（DEVA 派生は別ファイル） |
| `gradio_app_sam2_ben2_route_a_deva_for_Movie.py` | DEVA 方式再構成 + ルートA 動画デモ（port 7863・text_prompt 駆動） | GroundingDINO 内部検出のため手動 point/box canvas なし。`run_btn.click` 同期直結（ERR064） |
| `gradio_app_sam2_ben2_route_a_deva_manual_for_Movie.py` | DEVA 方式 + 手動 box/point（ポジ・ネガ）プロンプト動画デモ（port 7864） | 7863 に 7862 の prompt canvas を統合。`text_prompt` 任意（空=モードA 手動 seed のみ／入力=モードB 周期再検出併用）。手動 box/point は先頭フレーム seed。`run_btn.click` 同期直結（ERR064） |
| `gradio_app_sam2_ben2_tb_hybrid_deva_for_Movie.py` | DEVA 方式 + BEN2/TB ハイブリッド動画デモ（port 7865） | GroundingDINO→SAM2 で追跡した人物領域のみ transparent-background、領域外は BEN2。`BEN2TransparentHybridVideoExtractor` で alpha 合成方式（比較明 / 前後 over / screen）を UI 選択。 |
| `QUICKSTART_DEVA_BEN2_TB_HYBRID.md` | DEVA方式 BEN2/TB ハイブリッド動画アプリのクイックスタート | 起動方法、最小手順、主要パラメータ、RouteA ブラー誘導の実装箇所を記載。 |
...
- 推論時に自動取得される各種 checkpoint は、今後すべてローカル保存先へ永続化する。
- 基本フローは「ローカル優先 → 無ければ download → 同じローカルへ保存 → 次回以降は再利用」。
- 既定の保存先は `checkpoints/` 配下を使い、モデルごとのサブディレクトリで管理する（例: `checkpoints/BEN2`, `checkpoints/SAM2`, `checkpoints/transparent_BG`）。
- 永続化先は config から変更できる設計を維持し、コード内ハードコードは増やさない。
...
##### SAMURAI 動画伝搬の推奨設定（ERR049 / ERR050）

SAMURAI は Kalman filter による motion-aware memory を持つ **forward-only** tracker。低 VRAM GPU（Colab T4 16GB 等）で伝搬が `propagate 1/N` で凍結する stall を避けるため、以下を既定にする（すべて `config/inference_models.toml` の tracker entry 駆動）。

| 設定 | SAMURAI | 標準 SAM2 | 理由 |
|------|---------|-----------|------|
| `single_object_only` | `true` | `false` | SAMURAI fork の `_forward_sam_heads` は単一 obj(B=1)前提（`ious[0][best_iou_inds]`）で、複数 obj だと `Boolean value of Tensor ... ambiguous` で伝搬失敗。propagator が `run` 冒頭（warm_up 前）で複数 box を actionable に拒否（ERR051） |
| `offload_video_to_cpu` / `offload_state_to_cpu` | `true` | `false`（既定） | 常駐 VRAM を抑え stall 回避（ERR049）。標準 SAM2 は I/O オーバーヘッド回避のため無効維持 |
| `autocast_dtype` | `"float16"` | `"none"`（fp32 維持） | 伝搬を mixed precision で回し VRAM 削減・高速化（SAMURAI 本家と同じ）。標準 SAM2 は実績ある fp32 挙動を維持（ERR050） |
| `supports_bidirectional` | `false` | `true` | SAMURAI は逆走で KF の速度ベクトルが反転し追跡崩壊 + per-frame memory 2 倍。UI が `tracker_model.change`→`update_bidirectional_for_tracker` で双方向 checkbox を自動 OFF・無効化（ERR050） |
| プロンプト起点フレーム | 0（先頭） | 任意 | forward-only のため先頭起点が安定。末尾起点は逆走 stall を誘発 |

- propagator 側は `SAM2VideoPropagator.__init__(autocast_dtype, offload_video_to_cpu, offload_state_to_cpu)` + `_autocast_context(torch)`（cuda かつ dtype が `None/""/"none"` 以外のときのみ `torch.autocast`、他は `contextlib.nullcontext`）で実装。`run` は `with torch.inference_mode(), self._autocast_context(torch):`。
- SAMURAI は単一対象追跡専用。複数オブジェクトを切り抜く場合は標準 SAM2 tracker（`INFERENCE_TRACKER_VARIANT=sam2_facebook`）を使う（ERR051）。
- これらは UI（Gradio 冒頭 Markdown）と notebook 正本 `.py` 先頭セルにも明記済み。tracker 追加時もハードコード不要（registry のフィールドで制御）。
- 実機 Colab T4 での stall 解消・fp16 出力品質は要実測（ローカル .venv は torch/sam2/GPU 無しのため配線・契約検証のみ）。

#### SAMURAI / SAM2 Colab install（ERR045 / ERR041）

- 動画版 Colab Notebook は **facebook 版 sam2 ではなく同梱 SAMURAI fork（`samurai/sam2`）を installed `sam2` として導入する**。fork は `configs/sam2.1/` と `configs/samurai/` 両方と `samurai_mode` 対応モデルコードを含むため、facebook / SAMURAI 両 tracker を 1 つの sam2 で賄える。SAMURAI は訓練不要で標準 `sam2.1_hiera_large.pt` を再利用する（追加 DL 不要）。
- install は **非 editable + `--no-build-isolation`** を使う:
  `!{sys.executable} -m pip install --no-build-isolation "{PROJECT_ROOT}/samurai/sam2"`。
  - **非 editable（`-e` を付けない）**: Google Drive(FUSE) 上の editable install は `.pth`/egg-info 書き込みに失敗し `ModuleNotFoundError: No module named 'sam2'` になりやすい。非 editable なら pip が temp に複製してビルドするため回避できる。configs(`*.yaml`)は `MANIFEST.in` の `recursive-include sam2 *.yaml` + `include_package_data` で wheel に同梱される。
  - **`--no-build-isolation`**: 動画伝搬は `build_sam.py` が `++model.fill_hole_area=8` を強制し、`sam2/utils/misc.py` の `get_connected_components` が `from sam2 import _C`（CUDA 拡張、CPU fallback 無し）を要求する。build isolation 既定の `pip install` は分離環境に torch が無く `_C` がビルドされない（伝搬時に `ModuleNotFoundError: sam2._C`）。`--no-build-isolation` で現環境の torch を見せ、GPU ランタイムの nvcc で `_C` をビルドする。
- install cell は **fail-loud** にする: `!pip` は非ゼロ終了でも例外を出さないため、install 直後に `importlib.invalidate_caches()` → `importlib.util.find_spec("sam2") is None` なら actionable な RuntimeError を raise し、沈黙失敗が後段で cryptic 化するのを防ぐ。
- **`loguru` を明示 install する（ERR046）**: SAMURAI fork の `sam2/modeling/sam2_base.py` が `from loguru import logger` するが fork の `setup.py` の `install_requires` に無いため、fork を install しても入らない。Cell 1 で `!{sys.executable} -m pip install loguru` を明示導入する（未導入だと `build_sam2_video_predictor` で `ModuleNotFoundError: No module named 'loguru'`）。vendored fork は宣言されない実行時依存を持つことがあるため、import を grep し setup.py に無いものは notebook 側で install する。
- `_require_samurai_capable_sam2(config_name)`（`pipelines/components/video_model_components.py`）が `warm_up()` の build 直前で installed sam2 の `Path(sam2.__file__).parent / "configs" / "samurai"` を検査し、facebook 版検出時に fork 導入を促す RuntimeError を出す。
- 同一カーネルで先に sam2 を import 済みだと再 install が反映されない。Colab は「ランタイム再起動 → 上から順に全セル実行」を前提にする。
...
#### ID 追跡モデル（MOT）併用の意味（動画αマット計画）

動画αマットパイプライン（計画書 `計画書/2026-06-22_動画αマット_要件定義書.md`）では、SAM2.1 の上流に専用 ID 追跡モデル（ByteTrack / BoT-SORT）を置く。理由は SAM2.1 と役割が違うため。

- **追跡は SAM2.1 の構造的弱点**: SAM2.1 はセグメンタで、混雑・高速・自己遮蔽シーンでは固定窓メモリが誤差を伝播し ID 維持が崩れやすい（SAMURAI はその補正用の forward-only tracker）。
- **MOT は ID 維持に特化**: ByteTrack（高速）／ BoT-SORT（遮蔽・カメラ移動）は複数対象の ID 維持・再出現に特化した専用設計。ID 管理は MOT 側へ寄せる。
- **出力粒度が違う相補関係**: MOT=`track_id` 付き BBOX、SAM2.1=画素マスク。優劣ではなく粒度差。役割分担は「MOT=ID/追跡」「SAM2.1=画素シルエット＋box 内非対象除外（ポジ/ネガ点）」「BEN2=毛先のソフトα」。
- **`track_id` が下流を成立させる**: フレーム跨ぎで「どの crop／マスクがどの被写体か」を一貫保持でき、id 別α分離・BBOX crop の右紐付け（構成C）・ポイントプロンプト（F-5/F-6）の id 紐付けが成立する。`track_id` が不安定だと点が別人へ付き、id 別レイヤーが崩れる。
...
### 2-4-3. 長時間動画処理中の event SSE 接続維持（keep-alive）

- Colab/gradio.live の共有トンネルは event SSE に無通信が続くと idle 切断する。切れるとブラウザは pending 中の全出力を "Error" / "Connection errored out" にするが、サーバ側 Python は処理を継続する（stdout に例外なし、`propagate in video` が継続）。一次情報は Colab stdout（ERR048）。
- 動画パイプラインの低速ループ（SAM2 伝搬 / frame 準備 / transparent-background / tracking overlay）の進捗通知は `pipelines/components/video_model_components.py` の `_ProgressKeepAlive` で**時間ベース**に間引く（frame 数ベースにしない）。既定間隔 `_PROGRESS_KEEPALIVE_SEC=2.0` 秒で、frame 速度によらず無通信ギャップを抑え SSE を維持する。
- cv2 frame 書き込み（`VideoWriter._write_*`）は 1 frame が ms 単位で無通信ギャップが無いため keep-alive 対象外。
...
RGBA(透過)動画は `cv2.VideoWriter` では書けない（4ch を全 frame skip。FFmpeg "expected 3 channels but got 4"。ERR047）。透過動画は **imageio+ffmpeg**（`_ImageioAlphaVideoWriter`）で書き出す: `webm_vp9`=codec `libvpx-vp9` / pixelformat `yuva420p` / `output_params=("-auto-alt-ref","0")` / `macro_block_size=2`、`mov_png`=codec `png` / pixelformat `rgba` / `macro_block_size=1`。`_select_rgba_codec` は `_require_imageio()` で依存を確認し alpha 対応 imageio spec を返す（imageio[ffmpeg] が無い場合は握り潰さず連番(PNG)出力を促す `RuntimeError`）。alpha(1ch) / preview(3ch) は従来通り `cv2.VideoWriter`（`mp4v`）で書く。`imageio[ffmpeg]` は notebook Cell 1 で install 済み。`macro_block_size` は imageio-ffmpeg 正規パラメータで奇数解像度を偶数へ自動スケールする（yuva420p 要件）。

**実務上の運用方針（重要）**: 後段の編集（After Effects 等）が前提のため、実務では **PNG 連番出力（`rgba_sequence_dir` / `output_mode="sequence"`）だけで足りる**。透過動画（webm/mov）はあくまで確認用・補助的な成果物であり、品質・互換性・後工程の取り回しの観点では連番(PNG)が正本。透過動画書き出しが環境依存で不調な場合も、連番(PNG)で実務は完結する。
...
**任意フレーム起点の実行**: `prompt_frame_idx` が `max_frames` 以上でも範囲外エラーにせず実行できる。`run_video_background_removal` は `_effective_read_frames(max_frames, prompt_frame_idx) = max(1, max_frames, prompt_frame_idx+1)` で読み込み窓を prompt フレームまで自動拡張し、`video_reader` の `max_frames` へ渡す。これにより `SAM2VideoPropagator.run` の `prompt_frame_idx >= len(frames)` ValueError を回避する（ただし動画自体がその frame に満たない場合は依然 ValueError → gr.Error にラップ）。`max_frames` は「最低限処理する frame 数」の意味で、prompt が後ろにあるほど読み込み時間が増える。`run_tb_only_background_removal`（prompt 不使用）は対象外。旧来の fail-fast 範囲外バリデーションは撤廃済み（UX 改善要望）。
...
### 5-7. DEVA 方式再構成パイプライン（route_a_deva）

DEVA ライブラリは採用せず、その**方式**（疎結合な周期検出 island + SAM2 伝播 + consensus IoU マージ + メモリ整理）を `gradio_app_sam2_ben2_route_a_for_Movie.py` ベースで再構成する。`pipelines/route_a_deva_video_pipeline.py` の `build_sam2_ben2_route_a_deva_pipeline(tracker=None, extractor=None)` はベース RouteA と**同型配線**で、SAM2VideoPropagator を `DevaSemiOnlineTracker`（`pipelines/components/deva_semi_online_tracker.py`）に差し替える: `VideoReader → DevaSemiOnlineTracker → OwnershipResolver → BEN2RouteAVideoExtractor → VideoWriter / FrameSequenceWriter / TrackingOverlayWriter`。コンポーネント名は `deva_semi_online_tracker` / `ben2_route_a_video`。

**masks 出力契約（OwnershipResolver ドロップイン要件）**: `OwnershipResolver` は `frame_masks` を **per_object_logits のキーのみから再構築**し、欠落 frame を破棄する（`pipelines/components/ownership_resolver.py`）。そのため `DevaSemiOnlineTracker` は出力 `frame_masks` / `per_object_logits` のキーを **source_index（VideoReader の `metadata.metadata.sampled_frame_indices`）で統一**し、**全 source frame を per_object_logits で網羅**する（追跡対象なしクリップは `(0,H,W)` の per_object_logits を出して frame を残し、前景 0＝背景のみとして下流へ渡す）。これにより SAM2VideoPropagator と同一キー規約となりベース配線へドロップインできる。`DevaSemiOnlineTracker.run` の Note に同契約を明記する。

専用 Gradio アプリ `gradio_app_sam2_ben2_route_a_deva_for_Movie.py`（port 7863）は text_prompt 駆動で、GroundingDINO が内部検出するため手動 point/box canvas を持たない（DEVA 方式の忠実な簡素化）。`run_btn.click` は ERR064 同期直結。入力に `detection_every`（周期再検出間隔）・`max_missed_detection_count`（消失許容）・`iou_threshold`（consensus 突合閾値）を持つ。ベースアプリ `gradio_app_sam2_ben2_route_a_for_Movie.py` は**無改変を維持**する。

手動プロンプト併用版 `gradio_app_sam2_ben2_route_a_deva_manual_for_Movie.py`（port 7864）は、7863 に 7862 の手動 box/point（ポジ・ネガ）prompt canvas を統合した版。`DevaSemiOnlineTracker.run()` の `initial_boxes`/`initial_points`/`initial_labels`（後方互換・既定 None）で先頭フレーム seed を渡す。`text_prompt` 空＝**モードA**（検出島スキップ・単一クリップ全フレーム伝播＝基底相当）、入力あり＝**モードB**（先頭を box/point で精緻 seed→以降 text で周期再検出し consensus 復帰）。手動 box は track 事前登録＋`propagated_at_start` へ手動マスクを入れて重複採番を防止、手動 point は対象が動くため先頭クリップ(clip0)のみ適用（再投影しない）。アプリは union（複数 box）優先・single box fallback で `initial_boxes` を構築し、`text` 空かつ box 無し→`gr.Error`、point 単独（box 無し）→`gr.Error` で検証。`run_btn.click` は ERR064 同期直結。基底アプリ 7862/7863・`SAM2VideoPropagator`・`merge_consensus`（純関数）は**無改変**。

**検出島の複数 box → SAM2 画像モードの形状契約（ERR067）**: `SAM2Segmenter.run` に**複数 box（batched）**を渡すと SAM2 image predictor は `(K, C, H, W)`（box 数 K × multimask 候補 C）の4次元を返す。`run` は `predict` 直後に `masks.ndim == 4` のときだけ候補軸 C を score 最大で畳んで `(K, H, W)`・scores `(K,)` に正規化してから `build_mask_set`（`(N,H,W)` 契約）へ渡す。単一 box / points 経路の `(C, H, W)` 3次元出力はこの分岐を通らず無変更（基底アプリに影響なし）。検出島 `DetectionIsland._detect_one` はこの `(K,H,W)` を `normalize_segmenter_masks` でさらに吸収する。`build_mask_set` / `normalize_masks` は4次元を受理しない仕様なので、SAM2 に batched box を渡す箇所では必ず候補軸を畳んでから渡すこと。

**per_object_logits の解像度上限と host-RAM 有界化（ERR068）**: `DevaSemiOnlineTracker` は `global_per_object_logits`（frame ごとに `(N,H,W)` float32）を全 source frame 蓄積するため、対象数 N が多い（GroundingDINO の過検出）かつ高解像度（4K）だと host RAM が `numpy._ArrayMemoryError` で枯渇する（例: 19 box × 4096×2160 ≒ 641 MiB/frame）。これを `config/route_a.toml [deva] per_object_logits_max_side`（既定 1024・0 で無効＝原寸）で抑える。`DevaSemiOnlineTracker.run(per_object_logits_max_side=...)` は値 >0 のとき per-object チャネルを `cv2.resize`（INTER_AREA・空クリップ `(0,H,W)` も）で長辺 ≤ max_side に縮小し、**原寸を `masks["frame_hw"]=(H,W)` に保持**する。`OwnershipResolver` は `frame_hw` があれば前景 `frame_masks` を `cv2.resize`（INTER_LINEAR）で原寸へアップスケールしてから返す（BEN2 union ゲートは原寸 soft guard を要求）。`frame_hw` 不在時は従来どおり縮小しない（後方互換）。**per_object_logits は最終 α ではなく膨張＋ブラーされる低周波の soft guard** なので縮小しても実害は小さい（最終 α は BEN2 が原寸生成）。DEVA アプリは union 経路で config 値を、**per_object 経路では 0（原寸）**を渡す（所有権合成 `composite_alpha_by_ownership` が原寸一致を要求し、不一致時は `ValueError` を送出する＝silent でない）。overlay 用 `frame_masks`（tracker 内部）は原寸維持。
...
| 環境 | PROJECT_ROOT の値 |
|------|--------------------|
| Windows ローカル | `I:\AI_picasso\Matting-Anything`（`__file__.parent` / cwd）※2026-06-26 Google Drive から移行済み |
