...
| 項目 | 内容 |
|------|------|
| **動画1回の実行が2タイムスタンプフォルダに分裂＋overlay が output_mode を無視する不具合を共有コンポーネントで根治（ERR069・2026-07-01）** | **✅ 実装・TDD・レビュー完了（共有コンポーネント修正で全5動画アプリ一貫化・バックエンド出力パス修正）**。依頼（逐語要旨）: 「問題の本質はタイムスタンプじゃない `outputs\20260701_190932` 以下のフォルダに出力されているデータが sam2マスク確認用のデータで 本来お客が必要なデータが出力されていないことだ」。ユーザー確認事項: 連番モードは `<ts>/sequence/rgba/` `alpha/` のみ（preview 除外）／動画モードは提案通り（rgba・alpha・preview・tracking_overlay の webm）／overlay はトグルで出し分け（現状維持）／スコープ=共有コンポーネントを直して全アプリ一貫化。**真因**: 各 streaming extractor（`TransparentBGVideoExtractor`/`BEN2TransparentHybridVideoExtractor`/`BEN2RouteAVideoExtractor`）と `TrackingOverlayWriter` が**それぞれ独立に `datetime.now()`** で出力フォルダ名を決めており、処理が秒跨ぎになると extractor と overlay で別フォルダに分裂（成果物フォルダに overlay 無し／overlay フォルダに成果物無し）。さらに overlay は `enabled` のみで分岐し **`output_mode` を無視**して常に webm+PNG 両方を書いていた。**対処（後方互換・共有化）**: ①`video_common.py` に `resolve_run_timestamp(metadata)` 追加（`metadata["metadata"]["run_timestamp"]` を共有、無ければ now() で後方互換）。②`VideoReader.run()` が run 開始時に `run_timestamp` を metadata へ**1回だけ**注入。③3 extractor は `resolve_run_timestamp(metadata)` を使用（自前 now() と未使用 `import datetime` を撤去）。④連番から preview 除外（extractor は sequence で rgba/alpha のみ書き `preview_sequence_dir=None`、`FrameSequenceWriter` の透過 early-return も preview 非要求へ修正）。⑤`TrackingOverlayWriter.run()` に `output_mode` 引数追加＋`resolve_run_timestamp` 使用、video/both のみ `video/tracking_overlay.webm`、sequence/both のみ `sequence/overlay/*.png`（非該当は None）。⑥全5動画アプリの `tracking_overlay` 実行入力へ `"output_mode": output_mode` を配線（全 pipeline は既に `video_reader.metadata → tracking_overlay.metadata` 接続済み）。**検証**: `test_tracking_overlay.py` に5テスト追加（共有ts共有・output_mode video/sequence/both 準拠）。非 integration **392 passed / 3 deselected**（回帰なし）、5動画アプリ `--help` smoke 成功、get_errors=0。**サブエージェント(Explore)レビュー: 指摘0（MERGE 推奨）**＝全 pipeline の metadata 配線・全アプリの output_mode 供給・extractor の preview_dir 残参照無し・Hard rules 準拠・edge case（overlay 無効／metadata 欠落／日本語ラベル正規化）を確認。**RED→GREEN 実施**。UI 描画ではなくバックエンド出力パス修正のため ERR035 非該当（unit で検証可）。詳細は ERROR_LOG ERR069。 |
| **ハイブリッド DEVA 手動アプリ（port 7866）の SAM2 手動 box を「1個のみ」から「複数追加可能」へ改修（2026-07-01）** | **✅ 実装・TDD/smoke/レビュー完了（当該アプリ＋新規テストのみ・共有ヘルパー/他アプリ無改変）。UI fixed は Playwright 実機検証後に確定（ERR035）**。依頼（逐語要旨）: 「`gradio_app_sam2_ben2_tb_hybrid_deva_manual_for_Movie.py` は sam2 の BBOX が1つしか追加できない。複数追加できるようにすること。改修フローは copilot-instructions に従う。ハルシネーション対策も」。**根本原因**: 共有 `select_sam2_prompt`（`pipelines/components/ui_helpers.py`）は box 完成のたび `state["box"]`（単一）を上書きするため、Canvas で 2 個目を描くと 1 個目が消える。`state["boxes"]`（union）は Text→Box 補助専用だった。**設計（共有ヘルパー無改変・依頼外アプリ非波及）**: 共有 `select_sam2_prompt` は静止画版含む 19 ファイルで利用されるため無改変とし、当該アプリ内にローカルラッパー `select_sam2_prompt_multi` を新設。box モードで「新しい box の1点目クリック（box_buffer が 0→1）」かつ直前 box 確定済みのとき、直前 box を `state["boxes"]` へ重複排除で退避してから新規作図を開始。直近 box は `state["box"]` に残すため Extend（端延長）はそのまま使える。`prompt_canvas.select` を新ラッパーへ再配線。run ハンドラ `run_deva_hybrid_manual_background_removal` の `initial_boxes` を「union_boxes を list 化＋`manual_box` が未収録なら末尾追加」の重複排除結合へ変更（text 検出で box==boxes[0] を二重計上しない／Canvas 最後の1個も取りこぼさない）。`DevaSemiOnlineTracker.run()` は `for obj_id, box in enumerate(initial_boxes, start=1)` で複数 box を obj_id 1..M として扱えることを実コードで確認済み（tracker/pipeline 無改変＝I/O 契約不変＝疎結合維持）。prompt_mode Radio info と QUICKSTART に複数 box の説明を追記。**検証**: 新規 `tests/unit/test_hybrid_deva_manual_multi_box.py` **5 passed**（2個目描画で1個目を union 退避／3個蓄積／point で退避しない／run 結合 dedup／canvas が新ラッパーへ配線され共有関数直結でない）。`--help` smoke 成功、`get_errors`=0、`test_ui_helpers`＋`test_hybrid_alpha_components`＋`test_route_a_deva_hybrid_video_pipeline_wiring`＋新規 で **29→(＋配線)** 全 pass。**サブエージェント(code-review)**: Blocker/Major=0、Minor3（実ハンドラ経由テストでない旨→canvas 配線テスト追加で緩和／姉妹 route_a_deva_manual との initial_boxes 差分は無改変ゆえ regression でない／ERR035）、Nit2。**次アクション**: 複数 box 描画→追跡の Playwright 実機検証（モードA・B）で UI fixed を確定（ERR035・ユーザー指示待ち）。 |
| **親会社向け Haystack パイプライン機能説明書をルート直下に新規作成（2026-07-01）** | **✅ 完了（ドキュメントのみ・コード無改変）**。依頼（逐語要旨）: 「親会社に渡すためにHaystackのコンポーネントとパイプラインの機能説明の書類をつくりたい」「組み込み系エンジニアであるためわかりやすく伝えること」「図解はアスキーアートで」「過剰な略語をつかわないこと」「簡単な使い方の説明もつける」「系統別で区域分けすること」「文頭に機能を記した目次」「APIにするときこれを使えば楽だなって思えるように」。`.github/skills/haystack-pipeline/SKILL.md`・`REFERENCE.md`（8章・5-1〜5-6節）・`計画書/Haystack_pipeline計画表.md`（旧資料と明記）・`pipelines/` 配下の実ファイルを裏取りして `Haystack_パイプライン機能説明書.md` を新規作成。目次、全体像（Gradio/Haystack/PyTorch の3層構造 ASCII 図）、用語集、系統別説明（A:静止画/B:動画/C:BEN2+TBハイブリッド/D:モデルレジストリ）、呼び出し例、API化の利点、ファイル対応表で構成。**サブエージェント(code-review)レビューで2件の事実誤認を検出・修正済み**: (1) 動画呼び出し例で `VideoReader` の引数キーが実際は `path` でなく `video_path`、`VideoWriter` の出力は `video_path` キーでなく `matte` dict 経由（`rgba_video_path`等）と誤記していたのを修正。(2) 静止画系統（系統A）の図解・呼び出し例が単一パイプライン `build_sam2_tb_pipeline()` として描かれていたが、実際の `gradio_app_sam2_transparent_BG_haystack.py` では検出（単体呼び出し）→`build_sam2_maskset_pipeline()`→`build_mask_to_matte_pipeline()` の3段構成でアプリ層が結果を仲介する実態と食い違っていたため、図と呼び出し例を実装に合わせて修正。修正後、`SAM2Segmenter.run()` の引数名・`mask_set` 出力キー・`VideoWriter.run()` の `matte` 出力を実コードで再確認済み。RED/GREEN テストは対象外（文書のみ変更）。 |
| **DEVA方式 BEN2/TB ハイブリッド動画アプリ（port 7865）に手動 box/point（ポジ・ネガ）プロンプト機能を統合した新規アプリ（port 7866）を追加（2026-07-01）** | **✅ 実装・smoke/配線検証完了（新規ファイルのみ・基底アプリ無改変）。UI fixed は Playwright 実機検証後に確定（ERR035）**。依頼（逐語要旨）: 「`gradio_app_sam2_ben2_tb_hybrid_deva_for_Movie.py` には SAM2.1 の BBOX/point（ネガ・ポジ）プロンプト機能がない。`gradio_app_sam2_ben2_route_a_deva_manual_for_Movie.py` を参考に、`gradio_app_sam2_ben2_tb_hybrid_deva_for_Movie.py` をベースに `gradio_app_sam2_ben2_tb_hybrid_deva_manual_for_Movie.py`（当初はダブルアンダースコアだったが 2026-07-01 にシングルへリネーム）を新規作成し機能追加。Haystack component/pipeline の機能分割・疎結合・I/O 契約を守ること。計画書を計画書フォルダに日付+タイトルで保存。専用クイックスタートも作成」。**設計（パイプライン無改変）**: ハイブリッドパイプライン `build_sam2_ben2_tb_deva_hybrid_pipeline` は基底アプリと同じ `deva_semi_online_tracker` を使用し、既に `initial_boxes`/`initial_points`/`initial_labels`/`detection_start_frame` を受け付けるため、**パイプライン・component は一切改変せず**、tracker の入力ソケットへ手動 seed を渡すだけで要件を満たす（tracker の I/O 契約不変＝疎結合維持）。`segment-anything/`・`samurai/`・基底 hybrid（7865）・参考 manual route_a（7864）も無改変。**新規アプリ（port 7866）**: 基底 hybrid の構造（`_deva_per_object_logits_max_side()`・composition/tb_*/person_region_* の hybrid α 段・7-tuple 出力・codec 可視制御・gradio_client bool patch・win32 Proactor silencer）に、参考 manual の手動 prompt canvas（box/point pos・neg／個別削除／Extend／Text→Box 補助 GroundingDINO／clean-base-frame `prompt_base_image` gr.State=ERR063／検出起点フレームで Canvas 張り替え `extract_prompt_frame`）を統合。`text_prompt` を任意化し **モードA（手動 seed のみ・再検出なし）／モードB（text 併用の周期再検出）** を tracker が引数で自動判定。新規 `run_deva_hybrid_manual_background_removal` が prompt_state→`initial_boxes`(union 優先/single fallback)/`initial_points`/`initial_labels` を構築し「text または手動 box いずれか必須・point は box 必須」を `gr.Error` 検証、重い DEVA ロード前に `release_text_detector()` で VRAM 解放、`deva_semi_online_tracker` へ seed 注入。`run_btn.click` は ERR064 同期直結（25 inputs＝ハンドラ signature 完全一致・`prompt_state` は 2 番目・7 outputs）。**検証**: `gradio_app_sam2_ben2_tb_hybrid_deva_manual_for_Movie.py --help` smoke 成功、`get_errors`=0、関連 unit（`test_hybrid_alpha_components.py`＋`test_route_a_deva_hybrid_video_pipeline_wiring.py`）**13 passed**。**サブエージェントレビュー: マージ可（Blocker/Major/Minor なし）**＝inputs順⇔引数順25要素完全一致・outputs⇔return 7要素一致・pipeline dict キー基底一致・規約準拠・ERR063 コメントは参考アプリの誤記(ERR036)を正しく修正と評価。Nit 2件（`_estimate_processed_frames` の frame_step 未使用・`initial_labels` の `if points` 条件）は既存アプリ踏襲のため据え置き。**ドキュメント追加**: `計画書/2026-07-01_DEVA方式_BEN2_TBハイブリッド動画_手動boxpointプロンプト追加_実装計画.md`・`QUICKSTART_DEVA_BEN2_TB_HYBRID_MANUAL.md`。**RED 省略理由**: パイプライン/tracker/純関数は無改変（既存テスト済み）で、追加はアプリ層の UI 配線とパラメータ整形のみ＝挙動は既存 component が担保。**次アクション**: 実素材/短尺動画での Playwright 実機検証（run→出力5本 readyState=4／モードA・B 双方）で UI fixed を確定する（ERR035・以降はユーザー指示待ち）。 |
| **DEVA 方式 2 動画アプリ（手動 7864 / ハイブリッド 7865）に「検出起点フレーム（被写体が最大に映るフレームで seed/検出）」機能を追加＝双方向 backward pass で起点前フレームもカバー ＋ Playwright 実機検証 PASS で UI fixed 確定（2026-07-01）** | **✅ 完了（共有トラッカー拡張・後方互換・両 UI 実機検証 PASS）**。依頼（逐語要旨）: 「被写体が一番大きく映るフレームでプロンプトを入れたい」「`gradio_app_sam2_transparent_BG_haystack_for_Movie.py` にはその機能が実装されていたはず まずこちらを調査 正しく実装されていてかつ `gradio_app_sam2_ben2_tb_hybrid_deva_for_Movie.py` これにも実装可能なら機能を入れてほしい」。スコープは手動 DEVA アプリ（`gradio_app_sam2_ben2_route_a_deva_manual_for_Movie.py`・port 7864）にも拡張。**(0) 調査結論**: 透過 BG 動画アプリ（7861）は本機能を正しく実装済み＝手動 `prompt_frame_idx` スライダ＋`extract_prompt_frame`＋双方向トグル＋`_effective_read_frames` で読み取り窓を拡張、標準 SAM2 のみ（SAMURAI は forward-only で双方向自動無効）。**方式（ユーザー承認 Option A）＝検出起点フレーム＋双方向伝播**。**(1) 共有トラッカー拡張**: `pipelines/components/deva_semi_online_tracker.py` の `run()` に `detection_start_frame: int = 0`（既定 0＝現行挙動で後方互換）を追加。検出インデックスを `range(detection_start_frame, N, detection_every)`（text）／`[detection_start_frame]`（手動）へシフトし、clip0 を起点フレームで seed。**forward loop 後に `detection_start_frame > 0` なら backward pass**＝`SAM2VideoPropagator.run(frames=frames[:start+1], prompt_frame_idx=start, bidirectional=True)` で `[0, start)` を逆伝播し `global_frame_masks`/`global_per_object_logits` に書き込む（起点フレームは forward が担当＝重複回避、seed 無しは zero mask＋`(0,H,W)`）。バリデーション: `start<0 or start>=N` → ValueError、`start>0` かつ propagator が `single_object_only`（SAMURAI）→ ValueError。metadata に `detection_start_frame` 追加。**(2) 両アプリ UI＋配線**: 7864・7865 に「検出起点フレーム」Accordion（`gr.Slider(0,2000,step=1, elem_id="movie-detection-start-frame")`）＋起点フレーム表示ボタンを追加。7865 はプレビュー `gr.Image`、7864 は Canvas 張り替え（`extract_prompt_frame` が prompt_state をリセット）。`_effective_read_frames = max(1,max_frames,start+1)` で読み取り窓を拡張。run handler に `detection_start_frame` を `top_k` と `max_frames` の間へ挿入（inputs 順＝ハンドラ signature 完全一致・7864 は 24 inputs）。status に `start_frame=` を表示。**(3) TDD**: `tests/unit/test_deva_semi_online_tracker.py` に 6 件追加（起点シフト・backward がearly frames をカバー・start=0 で backward 無し・範囲外 raise・forward-only tracker raise・手動 box seed の backward が起点 prompt 使用）＝25 passed。wiring テスト 2 件を 24 inputs / slider label 検証へ更新。**非 integration 382 passed / 3 deselected**、両 `--help` OK、get_errors=0。**(4) サブエージェントレビュー APPROVE**（0 CRITICAL/HIGH/MEDIUM・2 LOW・3 COMMENT）＝local↔global マッピングに off-by-one 無し・`metadata=None` は local index keying・可変チャネル per_object_logits は OwnershipResolver が既に処理・forward-only gate 妥当を確認。LOW 2 件（トラッカー docstring・hybrid wiring test 対称性）反映済み＝35 passed。**(5) Playwright 実機検証 PASS（ERR035）**: `outputs/verify_manual_deva_start_frame.py`（port 7864・`inputs/17778552-hd_1920_1080_24fps.mp4`・`detection_start_frame=1`＋Canvas に box 描画＋`max_frames=3`）→ status「完了: … frames=3, 手動box=1, … start_frame=1 … total=13.4s」・出力 `<video>` 5 本すべて `readyState=4`・`videoWidth=1920`・`error=None`＝**backward pass に実 seed が流れる最強経路**を確認。`outputs/verify_hybrid_deva_start_frame.py`（port 7865・同動画・text="person"・`detection_start_frame=1`・`max_frames=3`）→ status「完了: … composition=lighten, frames=3, … start_frame=1 … total=19.7s」・`<video>` 5 本すべて `readyState=4`・`videoWidth=1920`・`error=None`。アプリログで forward＋backward の複数 `propagate in video` パスを確認。**留意**: 手動アプリの box 描画は Canvas 張り替え直後に settle 待ち＋クリックごとに bounding_box 再取得しないと `.select` イベントが取りこぼされる（初回 Timeout）。detection_start_frame スライダ number input は閉じた Accordion 内にあるため Playwright は先に Accordion ヘッダをクリックしてから `input[aria-label='number input for 検出起点フレーム（サンプリング後 index）']` を fill する。**後方互換**: `detection_start_frame=0` で従来完全一致。`segment-anything/`・`samurai/`・propagator・基底アプリは無改変。 |
| **ハイブリッド動画アプリ（port 7865）: 人物 mask ズレ時の無音フォールバック警告ログ追加 ＋ Playwright 実動画 UI 検証 PASS ＝ UI fixed 確定（2026-07-01）** | **✅ 完了（レビュー指摘反映＋ERR035 充足）**。依頼（逐語要旨）: 「review を行ってください」（`gradio_app_sam2_ben2_tb_hybrid_deva_for_Movie.py`）→ レビューは critical/major なし。追加依頼「ヒット率0の警告ログ追加は必要かな…日本語じゃないと見逃がしそうなので日本語での注意喚起もよろしく」「使う動画は inputs\8043439-uhd_2160_4096_25fps.mp4」で Playwright 実動画検証。**(1) 日本語警告ログ**: `pipelines/components/hybrid_alpha_components.py` の `BEN2TransparentHybridVideoExtractor.run()` に `logging`/`warnings` を導入し、frame_masks に有効 mask があるのに人物領域ヒットが 0（`person_hit_count == 0` かつ `available_mask_frames > 0`）＝**mask index ズレで TB が無音のまま全 frame BEN2 フォールバック**した場合と、mask が 1 件も無い場合を、それぞれ日本語で `logger.warning()` ＋ `warnings.warn(RuntimeWarning)` 通知（従来は握り潰さず沈黙していた潜在不具合を可視化）。metadata に `person_mask_hit_count`/`person_mask_available_frames`/`person_mask_fallback_warning` を追加し、アプリ側 `run_deva_hybrid_background_removal` が `person_mask_fallback_warning` を status 末尾へ `⚠️` 付きで表示。**(2) TDD**: `tests/unit/test_hybrid_alpha_components.py` に `_FakeBEN2`/`_FakeTB` と 3 件追加（index ズレで警告／mask 皆無で警告／正常ヒットで無警告、`recwarn` 使用）。hybrid file **10 passed**、非 integration **376 passed / 3 deselected**、get_errors=0。**(3) Playwright 実動画 UI 検証 PASS（ERR035）**: `outputs/verify_hybrid_deva_ui.py` で port 7865 の実アプリへ接続し `inputs/8043439-uhd_2160_4096_25fps.mp4`（4K）を upload、Text Prompt="person"、Accordion を開いて最大処理フレーム数=3 に設定し「Hybrid 実行」→ 完了まで待機。結果: status「完了: output_mode=video, composition=lighten, frames=3, text='person', … codec fallback: webm_vp9 ok 処理時間 total=202.1s」、出力 `<video>` 5 本すべて `readyState=4`・`videoWidth=2160`・`src` 充足・`error=None`、フォールバック警告なし（人物検出成立）。スクリーンショット `outputs/verify_hybrid_deva_ui.png`。**留意**: max_frames を UI で設定する number input は閉じた Accordion「Advanced: Hybrid α / 動画処理設定」内にあるため、Playwright 側で Accordion を先に開いてから `input[aria-label='number input for 最大処理フレーム数']` を fill する必要がある（初回は非表示で Timeout した）。GroundingDINO custom C++ ops 未ロード警告（CPU mode）は無害。ユーザーは「改修が必要なら私が指示します」＝以降の追加改修は指示待ち。 |
| **DEVA方式 + BEN2/TB ハイブリッド動画アプリ（port 7865）を追加（2026-07-01）** | **✅ 実装・配線検証完了（新規ファイル中心）**。`gradio_app_sam2_ben2_route_a_deva_for_Movie.py` をベースに `gradio_app_sam2_ben2_tb_hybrid_deva_for_Movie.py` を新規追加。GroundingDINO→SAM2.1→consensus の DEVA 方式で人物領域を追跡し、`BEN2TransparentHybridVideoExtractor` が **人物 mask 内だけ transparent-background、mask 外側は BEN2** で alpha を生成、最後に `lighten / person_over_ben2 / ben2_over_person / screen` から選んで合成する。新規 `pipelines/components/hybrid_alpha_components.py`（純関数 `compose_hybrid_alpha` / `build_person_region` + streaming Component）と `pipelines/route_a_deva_hybrid_video_pipeline.py` を追加し、Haystack の Component 境界・dict socket I/O・遅延モデルロードを維持。レビュー指摘（mask 空時に TB が全画面実行される、feather 境界で不透明 alpha が半透明化する）を修正し、mask 空時は TB 推論を skip、合成は full alpha を合成後に空間 blend する契約へ変更。検証: hybrid targeted 14 passed、非 integration **373 passed / 3 deselected**、`gradio_app_sam2_ben2_tb_hybrid_deva_for_Movie.py --help` OK、サブエージェント再レビューは significant issues なし。**ドキュメント追加**: `計画書/2026-07-01_DEVA方式_BEN2_TBハイブリッド背景除去アプリ_実装計画.md` と `QUICKSTART_DEVA_BEN2_TB_HYBRID.md` を追加。RouteA ブラー誘導は既存 `BEN2RouteAVideoExtractor._process_union_frame()` + `blur_background_outside_gate()` に実装済みと確認。Playwright 実動画検証は未実施のため、UI fixed 記録は実素材または短尺動画でのランタイム確認後に行う。 |
| **DEVA方式動画アプリに手動 box/point（ポジ・ネガ）プロンプト機能を統合した新規アプリ（port 7864）を追加（2026-06-30）** | **✅ 完了（Playwright 実機検証 PASS・新規ファイル中心・基底アプリ無改変）**。依頼（逐語要旨）: 「`gradio_app_sam2_ben2_route_a_for_Movie.py` のUIにあるような SAM2でのボックスプロンプト ポイントプロンプト（ネガティブ・ポジティブ）の機能はつけること可能ですか？…実装は `gradio_app_sam2_ben2_route_a_deva_for_Movie.py` これをベースに新しくファイルをつくること 計画書はtodoをのせて 計画書フォルダに日付+タイトル.mdで保存 ワークフローは `.github\copilot-instructions.md` `.github\skills\haystack-pipeline` を使うこと わからないことがあれば リポジトリのファイルもしくはウェブを検索してハルシネーション対策」。**設計（基底破壊なし）**: 基底アプリ 7862/7863・`SAM2VideoPropagator`・`merge_consensus`（純関数）・`segment-anything/`・`samurai/` は無改変。手動 point/label 情報は consensus の外（tracker 内部）に保持。**(1) tracker 拡張（後方互換・`pipelines/components/deva_semi_online_tracker.py`）**: `run()` に `initial_boxes / initial_points / initial_labels`（既定 None）追加。**モードA**（text 空＋手動 seed）= 検出島スキップ・`detection_frame_indices=[0]` の単一クリップで全フレーム伝播（基底アプリ相当・stale box/missed 削除を回避）。**モードB**（text 併用）= 通常の周期再検出に加え、先頭クリップ(clip0)だけ手動 point/label を SAM2 へ seed し、手動 box を obj_id 1..M で track 事前登録＋`propagated_at_start` に手動マスクを入れて consensus が text 検出と IoU 一致→重複採番を防止。バリデーション: text 空＋box 無し→raise、point 単独（box 無し）→raise、labels 長さ不一致→raise。point は対象が動くため clip0 のみ適用（再投影しない）。**(2) 新規アプリ `gradio_app_sam2_ben2_route_a_deva_manual_for_Movie.py`（port 7864）**: 7863 の構造（patch/helper/pipeline singleton/run/RouteA・DEVA 制御/7-tuple 出力）に、7862 の手動 prompt canvas（box モード／point モード pos・neg／個別削除／Extend／Text→Box 補助）と clean-base-frame `gr.State`（ERR063）を統合。`ui_helpers`（`select_sam2_prompt`/`remove_selected_*`/`draw_prompt_overlay`/`extend_box_to_edge`）を再利用。`text_prompt` を任意化し「text または手動 box のいずれか必須」を `gr.Error` 検証。`run_route_a_deva_manual_background_removal` が prompt_state→`initial_boxes`(union 優先・single box fallback)/`initial_points`/`initial_labels` を構築し `pipeline.run({"deva_semi_online_tracker": {...}})` へ注入。重い DEVA ロード前に Canvas 補助 GroundingDINO を `release_text_detector()` で解放。`run_btn.click` は ERR064 同期直結。**TDD**: RED→GREEN。tracker テスト `test_deva_semi_online_tracker.py` に 7 件追加（モードA 検出スキップ・単一クリップ・clip0 限定 point・hybrid 事前登録・各バリデーション）で**19 passed**。新規 `tests/unit/test_routea_deva_manual_movie_sync_wiring.py`（6 件: 同期直結＋overlay 出力・入力 23 個に prompt_state 含む・canvas select 配線・point 削除配線・builder import・singleton）。**非 integration 353 passed / 3 deselected**（回帰なし）、`--help` smoke OK、get_errors=0。**Playwright 実機検証 PASS（ERR035 充足）**＝port 7864 起動→ページ表示（canvas/Point Label/実行ボタン/Text Prompt 確認）→point/box モード切替で Point Label の表示・非表示が正しく切替→動画・prompt 無しで実行→`gr.Error`「先に動画をアップロードしてください。」トースト表示。**サブエージェント(Explore)レビュー: 承認適性あり（Critical/Major なし、Minor 3件は実装上問題なし＝seed_labels の明示デフォルト・status の pos/neg=0 表示・テストの入力数ハードコードは契約ロック目的で許容）**。**計画書**: `計画書/2026-06-30_DEVA動画アプリ_手動box-pointプロンプト追加_実装計画.md`。**実機の長尺動画での追跡品質はユーザーの実素材で要確認**。詳細は REFERENCE のアプリ一覧。 |
| **DEVA方式動画アプリ host-RAM OOM（`numpy._ArrayMemoryError`・約7分後クラッシュ）を per_object_logits 解像度上限化で根治（ERR068・2026-07-01）** | **✅ 完了（バックエンドのメモリ修正・ERR035 非該当）**。依頼（逐語要旨）: 「エラーログ\エラーログ_29.md このようなエラーがでた 詳しく分析して対処しなさい ちなみに エラーが出るまでに７分 対処フローは .github\copilot-instructions.md わからないことは リポジトリのファイルかウェブをしらべて ハルシネーションにならないようおねがい」。**真因（host-RAM 枯渇・VRAM ではない）**: `DevaSemiOnlineTracker` が `global_per_object_logits` として**フル解像度 float32 の per-object logits（N対象×H×W）を動画の全 frame 蓄積**。GroundingDINO が "person" を19 box 検出（top_k=20/box_threshold=0.25 既定）→4K（4096×2160）で 641 MiB/frame × 約190 frame ≒ 120GB 超を要求し `numpy._ArrayMemoryError`。クラッシュは propagator の `np.stack`（`video_model_components.py` L753）で表面化したが直接原因は tracker 累積。**ログ末尾の約200件 Haystack WARNING は無害なノイズ**＝Haystack の snapshot-on-exception が `progress_callback` クロージャ/ndarray をシリアライズ試行して出すもの（真トレースは末尾）。**対処（5箇所＋テスト・全後方互換・基底アプリ/propagator/segment-anything/samurai 無改変）**: ①`config/route_a.toml [deva] per_object_logits_max_side = 1024`。②`route_a_common.py` `_DEFAULT_ROUTE_A_CONFIG["deva"]`。③`deva_semi_online_tracker.py`＝`_downsample_per_object_logits` ヘルパ＋`run(per_object_logits_max_side=0)`（0=原寸=後方互換、>0 で per-object チャネルごと `cv2.resize` INTER_AREA 縮小、空クリップ `(0,H,W)` も縮小、原寸を `masks["frame_hw"]` に保持、`frame_masks`=overlay 用は原寸維持）。④DEVA アプリ＝`_deva_per_object_logits_max_side()` で config 値を tracker inputs へ注入（union は config 値、**per_object は所有権合成が原寸要のため 0**）。⑤`ownership_resolver.py`＝`frame_hw` があれば前景 `frame_masks` を `cv2.resize` INTER_LINEAR で原寸へアップスケール（BEN2 union ゲートが原寸 soft guard を要求）。**設計根拠**: per_object_logits は最終 α でなく膨張＋ブラーされる低周波の soft guard＝縮小しても実害小（最終 α は BEN2 が原寸生成）。4K×19対象で 641→約45 MiB/frame（約1/14）に削減し**非有界→有界**化。低 RAM 環境は max_side を 512 へ。**TDD**: RED→GREEN（`test_deva_semi_online_tracker.py` 4件・`test_ownership_resolver.py` 2件・`test_route_a_common.py` 更新）。**非 integration 347 passed / 3 deselected**（回帰なし）、get_errors=0、DEVA アプリ `--help` smoke 成功。**サブエージェントレビュー: マージ可（critical なし）**。major＝「per_object×max_side>0 の不変条件がアプリ層インライン三項のみ」は UI 到達不能かつ既存 `composite_alpha_by_ownership` が shape 不一致で `ValueError`（silent でない）＝follow-up・非ブロッカー。minor＝overlay 用 原寸 `frame_masks` 累積は残る（主因の約 1/N）。**ERR035 非該当**: UI レンダリング変更ではなくバックエンドのメモリ修正＝Playwright 実機検証不要（RED は単体テストで実施）。詳細は ERROR_LOG ERR068。 |
| **DEVA方式再構成 Phase 4: 実機検証（Playwright/ERR035）＋検出島の複数 box 4 次元マスク `ValueError` 根治（ERR067・2026-06-30）** | **✅ 完了（Playwright 実機検証 PASS・新規アプリ UI fixed 確定）**。依頼（逐語要旨）: 「次に進んでください」（= Phase 4 検証）→ 検証中にユーザー指摘「ポート開けてないんじゃないか？」。**ポート問題は誤検知**: `gradio_app_sam2_ben2_route_a_deva_for_Movie.py`（port 7863）は `127.0.0.1:7863` で LISTEN・HTTP 200 を確認済み。Playwright も接続成功し動画 upload + text_prompt 設定まで到達していた。Tee-Object のバッファで起動ログが空に見えただけ。**真の不具合（ERR067）**: DEVA アプリを text_prompt="person" で実行すると検出島が約3.4秒で `gr.Error` 終了し、Playwright が「完了:」を30分待ってタイムアウト（=ハングではなく即時エラーの取り違え）。スタック要点 `ValueError: masks は (H,W) または (N,H,W) 形式である必要があります: shape=(16, 3, 4096, 2160)`。**原因**: `SAM2Segmenter.run` は `boxes`（複数 box / batched）を構文上受け付けるが、SAM2 image predictor は複数 box で `(K, C, H, W)`（box 数 K × multimask 候補 C）の4次元を返す。これを候補軸 C で畳まず `build_mask_set`→`normalize_masks`（(H,W)/(N,H,W) のみ受理）へ渡し4次元で `ValueError`。検出島の `normalize_segmenter_masks`（4次元畳み込み helper）到達前に segmenter 内部で落ちていた。基底アプリは常に単一 box / points（3次元）でしか呼ばないため未顕在だった。**対処（根治）**: `pipelines/components/model_components.py` の `SAM2Segmenter.run` で `predict` 直後に「`masks.ndim == 4` のときだけ」候補軸 C を score 最大で畳んで `(K,H,W)`・scores を `(K,)` に正規化（`C==1` は `[:,0]`、`C>1` は box ごと `argmax(score)`）。単一 box / points の3次元出力はこの分岐を通らず**完全無変更**（基底アプリ・既存テストに影響なし）。再発防止として Playwright スクリプト `outputs/verify_routea_deva_sync_output.py` の `wait_for_function` 条件に「失敗しました / Error」を追加しエラー時の長時間ハングを排除。**TDD**: RED→GREEN 新規 `tests/unit/test_sam2_segmenter_batched_boxes.py`（3件: `(K,C,H,W)`→`(K,H,W)` 畳み込み＋best 候補選択・`C==1`・単一 box 非対象）。**非 integration 342 passed / 3 deselected**（339→342・回帰なし）、get_errors=0。**Playwright 実機検証 PASS**＝修正後アプリ再起動→`verify_routea_deva_sync_output.py`（port 7863・samurai_demo.mp4・text_prompt="person"）で出力動画5本すべて src 充足・status「完了:／処理時間」表示。アプリログで SAM2 伝播がクリップ 11/11/10 frame（1 frame 重複）で semi-online 動作・consensus 経由・VP9/webm 書き出し成功・"SAMURAI mode: False"（標準 SAM2）を確認。短尺デモは数秒で完走し「1分動画≤50分」予算に対し大幅余裕。**ERR035 充足＝新規 DEVA アプリ UI を fixed と確定**。詳細は ERROR_LOG ERR067。残: 真の1分動画での実測はユーザーの実素材で要確認（短尺で機能・性能傾向は確認済み）。 |
| **DEVA方式再構成 Phase 3: route_a_deva パイプライン配線 + 専用 Gradio アプリ TDD 実装（2026-06-30）** | **✅ 完了（GPU 非依存・新規ファイルのみ・ベースアプリ無改変）**。依頼（逐語要旨）: 「phase3よろしくお願いします わからないことがあれば リポジトリの関連ファイルやウェブを検索して ハルシネーションを避けるように」。**最重要前提（外すと破壊）**: DEVA ライブラリは採用しない。DEVA の**方式**（疎結合な検出island＋task非依存伝播＋consensus＋メモリ整理）を `gradio_app_sam2_ben2_route_a_for_Movie.py` ベースで再構成。**実装**: ①`pipelines/route_a_deva_video_pipeline.py`（新規）= `build_sam2_ben2_route_a_deva_pipeline(tracker=None, extractor=None)`。ベース RouteA 配線と**同型**だが SAM2VideoPropagator を `DevaSemiOnlineTracker` に差し替え＝`VideoReader → DevaSemiOnlineTracker → OwnershipResolver → BEN2RouteAVideoExtractor → VideoWriter/FrameSequenceWriter/TrackingOverlayWriter`。コンポーネント名 `deva_semi_online_tracker`/`ben2_route_a_video`。②`gradio_app_sam2_ben2_route_a_deva_for_Movie.py`（新規・port 7863）= text_prompt 駆動 UI（DEVA は GroundingDINO で内部検出するため手動 point/box canvas は不要＝忠実な簡素化）。入力: input_video・text_prompt（必須）・detection_every(1-60,既定10)・max_missed_detection_count(0-10,既定3)・iou_threshold(0.05-0.95,既定0.5)・matte_mode・Advanced(GroundingDINO box/text/top_k・RouteA dilation/blur/feather/refine/gate_alpha/mask_floor/max_frames/frame_step/output_mode/rgba_codec/output_type/overlay)。出力 7-tuple（rgba/alpha/preview/tracking_overlay/sequence_files/sequence_dirs/status）。`run_btn.click` は ERR064 同期直結。**契約の要（OwnershipResolver 経路）**: OwnershipResolver は `frame_masks` を **per_object_logits のキーのみから再構築**し欠落 frame を破棄するため、`DevaSemiOnlineTracker` の出力 frame_masks/per_object_logits キーを **source_index（VideoReader の sampled_frame_indices）で統一**し、**全 source frame を per_object_logits で網羅**（対象なしクリップは (0,H,W)）するよう改修。これによりベース配線へドロップイン可能。**規約準拠**: warm_up 遅延構築/torch.load なし/try-except:pass なし/raise・gr.Error 通知（video_path/text_prompt/detection_every を fail-fast）。**TDD**: RED→GREEN。新規 `tests/unit/test_route_a_deva_video_pipeline_wiring.py`（4件: 配線同型・tracker↔ownership/overlay エッジ・tracker/extractor 注入）＋ `tests/unit/test_routea_deva_movie_sync_wiring.py`（3件: 同期直結＋overlay 出力・builder import・singleton キャッシュ）＋ tracker テストに per_object_logits 全フレーム被覆 2件追加。**サブエージェント(Explore)レビュー**: 必須1件（空クリップで per_object_logits 欠落→OwnershipResolver が frame 破棄）＋推奨1件（docstring に source_index キー規約明記）を**反映済み**＝空クリップで (0,H,W) per_object_logits をゼロ埋め・run docstring に Note 追加。**非 integration 339 passed / 3 deselected**（Phase 2 の 337 から +2、回帰なし）、DEVA アプリ `--help` smoke 正常、get_errors=0。**ERR035 留意**: UI「fixed」確定には実機 Playwright 検証が必要＝現時点は「実装完了」止まり（次は Phase 4 実測＋Playwright）。 |
| **DEVA方式再構成 Phase 2: DevaSemiOnlineTracker（コーディネータ）TDD 実装（2026-06-30）** | **✅ 完了（GPU 非依存・新規ファイルのみ・既存改変なし）**。依頼（逐語要旨）: 「続行お願いします！」（= Phase 2 実装）。**最重要前提（外すと破壊）**: DEVA ライブラリは採用しない。DEVA の**方式**（疎結合な検出island＋task非依存伝播＋consensus）を `gradio_app_sam2_ben2_route_a_for_Movie.py` ベースで再構成。**実装**: `pipelines/components/deva_semi_online_tracker.py`（新規 @component）= DEVA semi-online を**クリップ周回**で束ねる中核。検出島(DetectionIsland)・伝播(SAM2VideoPropagator)・consensus(merge_consensus 純関数) を内部で周回し、**再シード状態（track memory: tracks/next_object_id/propagated_at_start）をコンポーネント内部に隔離**＝外部 DAG は前向き疎結合を維持（フィードバック隔離）。`run(frames, metadata, text_prompt, detection_every, max_missed_detection_count, iou/box/text 閾値, top_k, progress_callback) → {"masks": <BEN2 union 契約>}`。流れ: ①`detection_frame_indices = range(0, N, detection_every)` で検出島を1回実行 ②各クリップ `[d_i, d_{i+1}]`（次検出フレームを **1 frame 重ね**て伝播）で `merge_consensus`（直前クリップが d_i へ伝播したマスク vs 検出を IoU 突合→維持/新規採番/missed++/削除）→ box を持つ track を SAM2 再シード ③クリップ末の per-object logit(>0) を次 consensus の propagated に保持 ④`build_frame_mask_sequence` で union 契約に集約（metadata に detection_frame_indices/new_object_ids/deleted_object_ids/num_objects）。**契約検証**: per_object_logits キーは `metadata=None` 指定で local 0..len-1（実コード video_model_components L656 `source_indices=range(len(frames))` 確認済）。**規約準拠**: warm_up 遅延構築（import 時重初期化なし）/torch.load なし/try-except:pass なし/raise でエラー通知（frames 空・detection_every<=0・text_prompt 空・SAMURAI(single_object_only)×複数 seed は ERR051 早期 raise）。**TDD**: RED（ModuleNotFoundError）→ GREEN。新規 `tests/unit/test_deva_semi_online_tracker.py`（8件: union 契約一致/検出は検出フレームのみ/detection_every がクリップ数を制御/重なりで object_id 継続/離れた box で新規採番/max_missed 超過で削除/frames 空 raise/detection_every=0 raise）。**非 integration 330 passed / 3 deselected**、get_errors=0。**サブエージェント(Explore)レビュー**: マージ可（必須2点反映済み＝per_object_logits の local キー前提を docstring 明示・SAMURAI 複数対象の早期 raise ガード追加）。UI 変更なし＝ERR035 非該当。次は Phase 3（route_a_deva_video_pipeline.py + gradio_app_sam2_ben2_route_a_deva_for_Movie.py、ベースアプリ無改変）。 |
| **DEVA方式再構成 Phase 1: 純関数 Component 2本（ConsensusMerger / DetectionIsland）TDD 実装（2026-06-30）** | **✅ 完了（GPU 非依存・新規ファイルのみ・既存改変なし）**。依頼（逐語要旨）: 「続けてください」（= Phase 1 実装）。**最重要前提（外すと破壊）**: DEVA ライブラリは採用しない。DEVA の**方式**（疎結合な検出island＋task非依存伝播＋consensus）を `gradio_app_sam2_ben2_route_a_for_Movie.py` ベースで再構成。**(1) ConsensusMerger 純関数** `pipelines/components/consensus.py`（新規・モデル/IO を持たない純関数のみ）: `compute_mask_iou`（union 空は 0.0）/`match_by_iou`（IoU 降順 greedy・同点は (p_idx,d_idx) 昇順で決定的）/`build_detection_masks`/`merge_consensus`（matched→missed=0 + box/label/score 再アンカー、未マッチ検出→新 object_id 採番 + reseed、未マッチ track→missed++ で max_missed 超過時 delete。**入力 track を破壊変更しない**＝各 dict をコピー）。track 契約 `{object_id, missed, mask(H,W)bool, box|None, label|None, score}`、detected 契約 `{masks(K,H,W)bool, boxes(K,4), scores(K,), labels[str]}`。**(2) DetectionIsland @component** `pipelines/components/detection_island.py`（新規）: GroundingDINO（text→box）→ SAM2 画像モード（box→mask）を**検出フレームのみ**実行（周期再検出 detection_every）。依存（detector/segmenter）注入可能でテスト容易・差し替え容易、`warm_up` で実モデル（GroundingDINOMultiBoxDetector/SAM2Segmenter）を遅延構築（import 時に重い初期化なし）。`run(frames, detection_frame_indices, text_prompt, box/text/iou/top_k) → {detections:{frame_idx:{masks(K,H,W)bool, boxes(K,4), scores(K,), labels[str]}}}`。検出ゼロフレーム（対象が一時的に隠れる等）は detector の `ValueError("検出されませんでした")` を**その語句のときだけ**空エントリ `masks(0,H,W)` へ変換（他 ValueError は素通し＝握り潰さない）、範囲外 index は `ValueError`。`@component.output_types(detections=dict)` で socket 型宣言。SAM2 出力形状の差を吸収する純関数 `normalize_segmenter_masks`（(K,1,H,W)/(K,C,H,W: score最大選択)/(K,H,W)/(H,W) → (K,H,W) bool）も同梱。**TDD**: RED（`ModuleNotFoundError` / collection error）→ GREEN。新規 `tests/unit/test_consensus.py`（18件: IoU 同一/disjoint/空/部分/soft-float、match 単一/閾値未満/greedy/余剰検出/**同点 tie-break 決定性**、build 抽出/空、merge matched-reset/new-detection-reseed/unmatched-missed++/delete-after-max/keep+add 同時/**no-mutation**）＋ `tests/unit/test_detection_island.py`（10件: normalize 4形状[**K>1×C>1 multimask 含む**]、検出フレームのみ実行、出力契約形状、segmenter へ box 伝達、空エントリ、範囲外 raise）= **28 passed**。**非 integration 320 passed / 3 deselected**（回帰なし）、新規4ファイル `get_errors`=0。**サブエージェント（Explore）レビュー実施**＝高品質評価・ブロッカーなし。指摘反映: ①`normalize_segmenter_masks` の 3D 分岐にコメント明確化 ②(K>1,C>1) multimask テスト追加 ③match 同点決定性テスト追加。Low（boxes/confidences の reshape は防御的冗長＝害なしで温存）。**ERR035 非該当**: UI 変更なし＝Playwright 不要（純関数 Component の I/O 契約検証）。**次アクション**: Phase 2＝`DevaSemiOnlineTracker`（@component coordinator・clip ループ + 途中再シード状態を内部保持・出力は BEN2 union 契約 `{frame_masks:{idx:(H,W)float32}, object_ids:list[int], metadata}` に整合）。その後 Phase 3（`pipelines/route_a_deva_video_pipeline.py` + 新規 `gradio_app_sam2_ben2_route_a_deva_for_Movie.py`・base アプリは無改変）、Phase 4（検証: --help smoke / 1分動画<50min 実測 / Playwright / レビュー）。 |
| **DEVA方式再構成 Phase 0: 既存コード前提検証＋調査書 SAMURAI 単一対象明記（2026-06-30）** | **✅ 完了（コード変更なし・調査/計画ドキュメント更新と読み取り検証のみ）**。依頼（逐語要旨）: 「samuraiは単一オブジェクトのみの追跡しかできない旨を書いておいてください このファイル後今後のノイズになると嫌なので」「phase0 開始してください 不確かなことがあれば このリポジトリやwebの調査をして ハルシネーションをさけてね」。**最重要前提（外すとプロジェクト破壊）**: DEVA ライブラリは採用しない。DEVA の**方式**（疎結合な検出island＋task非依存伝播＋consensus）を `gradio_app_sam2_ben2_route_a_for_Movie.py` ベースで再構成する。**(1) 調査書更新**: `調査/2026-06-30_動画マッティング_追跡はがれ再検出復帰_手法調査.md` の §1要約・§8-1・§8-3表・§8-4・§8-5 を改訂し「**SAMURAI は単一オブジェクトのみの追跡（確定）**」を明記。根拠は本リポジトリ実コード `samurai/scripts/demo.py`＝`add_new_points_or_box(box=bbox, frame_idx=0, obj_id=0)` で **obj_id=0 単一のみ**初期化・`color=[(255,0,0)]` 単色・KF/motion-aware memory は単一対象前提・評価ベンチも全て単一対象 VOT。複数被写体は SAMURAI ではなく SAM2 per-object VOS を使う、と固定。**(2) Phase 0 コード前提検証（3点・実ファイル確認済）**: ①**`SAM2VideoPropagator`（`pipelines/components/video_model_components.py` L515-779）は途中再シード未実装**＝`run()` は `prompt_frame_idx`（既定0）で `add_new_points_or_box` を**1回だけ**呼び、box/boxes を obj_id 1..N 一括登録後 `propagate_in_video`。周期検出で途中に新 obj を add する経路は無い。**ただし下層 SAM2 の `add_new_points_or_box(frame_idx=...)` は任意フレーム対応**なので、DEVA方式の「detection_every ごとに途中シード＋再伝播」は**新コンポーネント（DevaSemiOnlineTracker）側で実装が必要**（既存 propagator はそのまま流用不可、ラップ新設）。②**`SAM2Segmenter`（`pipelines/components/model_components.py` L475-560, 画像モード）の box 契約 OK**＝`run(image, points, labels, box, boxes, multimask)` で単一 `box:list[int]` も複数 `boxes:np.ndarray` も `predict(box=...)` に渡せる→GroundingDINO の boxes(K,4) を `boxes` に渡せば DetectionIsland（text→box→mask）が成立。出力 `masks(K,H,W)/scores/mask_set/diagnostics`。③**BEN2 union 契約 OK**＝`BEN2RouteAVideoExtractor`（`pipelines/components/ben2_components.py` L190-）は union/per_object 経路で frame_masks union dict を消費。ConsensusMerger の出力をこの union 契約に合わせれば BEN2 流用可能。**結論**: DetectionIsland と ConsensusMerger を新規 @component で作り、伝播は既存 SAM2 を**新ラッパー（途中シード対応）**で包む方針が確定。**ERR035 非該当**: UI 変更なし＝Playwright 不要。**RED 省略理由**: コード挙動を変えていない（ドキュメント更新と読み取り調査のみ）。**次アクション**: Phase 1（DetectionIsland @component の TDD 実装）。RTX4090 実測（GroundingDINO 検出時間／Route A matting コスト）は実装と並行。 |
| **トラッカー B+（sam2.1_hiera_base_plus.pt 未配置）選択時の分かりにくい FileNotFoundError を warm_up の fail-fast 検証で根治＋ネガティブ点疑惑の切り分け（ERR066・2026-06-27）** | **✅ 完了（バックエンド fail-fast・ERR035 非該当）**。依頼（逐語要旨）: 「ネガティブポイントプロンプトをポジティブで伝えている疑惑がある また トラッキングモデル二つと sam2.1 の連携が取れていない気がする」。レビューは GPT-5.5（本タスクはサブエージェントレビュー未実行）。**(1) ネガティブ点はバグなし**: UI Radio(negative)→`select_sam2_prompt` で `label=0` を `points` と同時に `state["labels"]` へ append→`run_route_a_background_removal` が `labels` を `SAM2VideoPropagator` へ→`add_new_points_or_box(labels=np.int32([...0...]))` まで `0` を保持（box 併用時も `assign_points_to_boxes`＋`assigned_labels` で label 保持）。**負点は負点として正しく SAM2 に伝達**。「効かない」体感の主因は ERR064 で明文化済みのルートA設計＝SAM2 マスク（point の反映先）は背景ブラーのゲート G 専用、最終 α は BEN2 単独生成（マスク入力ポート無し）。`gate_alpha=ON` で point が α に効く。Tracking Overlay が point で変われば SAM2 伝達は検証可（hint で案内済み）。**(2) トラッカー連携の実害**: registry は SAM2.1/SAMURAI の Large/B+ 計4 tracker を提供し、`get_route_a_pipeline` が id ごとに `config_name`/`checkpoint_path` で個別構築＝配線は正しい。同梱 SAMURAI fork は `configs/sam2.1` と `configs/samurai` 両方を持ち4 tracker 利用可。**だが `checkpoints/SAM2/` に `sam2.1_hiera_large.pt` のみ存在、B+ が参照する `sam2.1_hiera_base_plus.pt` が未配置**→B+ 選択で `build_sam2_video_predictor`→`torch.load(missing)` が約9秒後に深い `FileNotFoundError`＝「連携が取れていない」感覚の実体。**対処**: `SAM2VideoPropagator.warm_up` 冒頭（冪等 early-return 直後・`require_gpu`/sam2 import 前）に `Path(checkpoint_path).is_file()` の fail-fast 検証を追加し、欠落ファイル名＋代替（Large）を示す `RuntimeError` を送出（全アプリ共通に効く）。ネガティブ点側はコード変更なし（設計説明のみ）。**検証**: RED→GREEN 新規 `test_movie_runtime_bugs.py::test_warm_up_missing_checkpoint_raises_actionable_error`（存在しない checkpoint で即時 `RuntimeError`、メッセージにファイル名・"checkpoint" を含む。RED 時は約9秒の torch `FileNotFoundError`→GREEN で約5秒の明確エラー）。**非 integration 294 passed / 3 deselected**、変更2ファイル `get_errors`=0。**ERR035 非該当**: UI レンダリング変更ではなくバックエンドの error-path 検証のため Playwright 実機検証不要。**恒久対応の選択**: B+ を使うなら `sam2.1_hiera_base_plus.pt` を `checkpoints/SAM2/` に配置／使わないなら現状の明確エラーで Large を案内（dropdown 非表示化はスコープ外）。詳細は ERROR_LOG ERR066。 |
| **RouteA/TB 動画: Alpha/Preview/Tracking Overlay が UI に表示されない（cv2 'mp4v' 非対応コーデック）を VP9/webm streaming writer で根治（ERR065・2026-06-26）** | **✅ 完了（Playwright 実機検証 PASS）**。依頼（逐語要旨）: 「進捗アイコンのみ改善 他はアルファビデオ preview video tracking overlay 追跡確認用すべて表示されていない / gradioUI あるすべてのパラメーターとウィンドの導線が不具合なくつながり 正しくアウトプットできていること かつ 各種アウトプットのディレクトリが正しくあることを確認 / review は gpt5.5 が行うこと / `.github\copilot-instructions.md` にしたがって改修」。**真因**: ERR064 で同期直結に戻し出力が届くようになった後も、RGBA だけ `_ImageioAlphaVideoWriter`（imageio+ffmpeg, webm/VP9）で再生でき、Alpha/Preview/Tracking Overlay は **cv2 `VideoWriter` の `'mp4v'`（MPEG-4 Part 2）** で書き出していた。MPEG-4 Part 2 は Chromium/`gr.Video` でデコード不可（`MediaError code 4`）＝ファイルは生成されても画面に出ない潜在不具合が、ERR064 で出力到達後に表面化。**UI 配線監査（依頼②）**: `run_btn` の inputs 19 個＝`run_route_a_background_removal` のシグネチャ（progress 除く 19）と順序一致、outputs 7＝戻り 7-tuple 一致。BEN2 のみタブ inputs 7／outputs 6＝戻り 6-tuple 一致。`.change`/`.then`/`.click`（codec 表示・point label 表示・bidirectional・prompt frame 抽出・点/箱削除）も整合。配線バグ無し＝症状は純粋にコーデック。**出力ディレクトリ（依頼③）**: 各 writer の ctor / `write_png_frame` が `path.parent.mkdir(parents=True, exist_ok=True)` で `video/` と `sequence/{rgba,alpha,preview}` を都度生成（新 webm writer も同様）。**対処**: ブラウザ再生互換の VP9/webm streaming writer `_ImageioWebmVideoWriter`（`codec="vp9"`, `pixelformat="yuv420p"`, `output_params=["-auto-alt-ref","0"]`, `macro_block_size=2`; grayscale→RGB複製/RGBA→先頭3ch）を `pipelines/components/video_model_components.py` に追加。cv2 `'mp4v'` の5箇所（BEN2 alpha/preview、TB alpha/preview、TrackingOverlayWriter overlay）を置換し拡張子を `alpha.webm`/`preview.webm`/`tracking_overlay.webm` に変更（VP9 は mp4 コンテナ非互換）。RGBA(webm/VP9) と統一。`_require_imageio()` の例外契約維持。**検証**: RED→GREEN 新規 `tests/unit/test_browser_playable_video_writer.py`（ffmpeg プローブで `vp9` 確認・grayscale→3ch 確認）。既存 `test_movie_runtime_bugs.py::test_transparent_bg_video_rgba_streams_4channel_to_imageio` を新コーデック契約へ更新。**非 integration 293 passed / 3 deselected**、RouteA `--help` exit 0、get_errors=0。**Playwright 実機 PASS**＝`outputs/verify_err065_video_playback.py`（Chromium 148）: 実生成 preview/alpha/overlay webm を `<video>` 読込→`readyState=4, videoWidth>0, error=null`（再生可）／対比で旧 cv2 'mp4v' は `error=4, videoWidth=0`（再生不可）を確認（ERR035）。**レビュー**: GPT-5.5 が実施（本タスクではサブエージェントレビュー未実行＝依頼指示）。**教訓**: `gr.Video` は webm/VP9・H.264・AV1 のみ再生可。cv2 既定 `'mp4v'`(MPEG-4 Part 2) は不可。H.264 は Playwright バンドル Chromium でデコード不可な場合があるため、実機検証可能性と RGBA 一貫性を優先し VP9/webm に統一。詳細は ERROR_LOG ERR065。 |
| **RouteA 動画: alpha/preview/tracking overlay の出力が UI に出ず推論プログレスも出ない不具合を同期直結へ戻して根治（ERR064・2026-06-26）** | **✅ 完了（Playwright 実機検証 PASS）**。依頼（逐語要旨）: 「alpha video preview video tracking overlay それぞれのUIに出力が無い / 推論時のプログレスアイコンも出ない / ネガティブポイントプロンプトの伝播が無い / トラッキング方法をレファレンスで確認後 不具合に対処 / `.github\copilot-instructions.md` に従い対処」。**確認した方針（ユーザー回答）**: 実行環境=ローカルのみ(127.0.0.1, トンネル不使用)／修正方針=A: 同期実行へ戻す（ローカル前提・推奨）／negative=overlay 自体が出ず確認不能。**真因**: ERR058 の非同期ジョブ方式（`JobManager`+`gr.Timer` ポーリング、gradio.live トンネル SSE 切断対策）の Timer ポーリングが UI に出力・進捗を届けず空のまま。ローカルではトンネル切断が無く非同期方式は不要。同期 click でないため Gradio 標準プログレス（スピナー）も出ない。negative point の伝播自体はデータ経路（`select_sam2_prompt` が label=0 を state 保存→`run_route_a_background_removal` が `labels` を `sam2_video_propagator`→`add_new_points_or_box(labels=...)`）に問題なく、overlay 非表示の二次症状。**対処（最小差分・配線のみ）**: ①`run_btn.click(start_route_a_job, outputs=[job_id,status,timer,btn])`+`route_a_timer.tick(poll_route_a_job,...)` を撤去し `run_btn.click(run_route_a_background_removal, inputs=[...19...], outputs=[rgba_video, alpha_video, preview_video, tracking_overlay_video, sequence_files, sequence_dirs, run_status])` へ同期直結。②BEN2 のみタブも `route_a_only_run_btn.click(run_route_a_only_background_removal, outputs=[...6...])` へ同期直結し `.tick` 撤去。③非同期関数群（`start_*_job`/`poll_*_job`/`_ProgressBridge`/`_JOB_MANAGER`）と timer/job_id コンポーネント・`JobManager` import は Colab 再利用に備え温存（未配線=無害）。fail-fast 検証は `run_route_a_background_removal` 内部に残存し維持。**検証**: RED→GREEN 新規 `tests/unit/test_routea_movie_sync_wiring.py`（`demo.fns` 内省で同期直結＋Tracking Overlay 出力ラベルを固定・2件）。**非 integration 291 passed / 3 deselected**、`--help` smoke 正常、get_errors=0。**Playwright 実機 PASS**＝`outputs/verify_routea_sync_output.py`（port 7862, samurai_demo.mp4 30 frames, box+negative point→完了表示＋出力 `<video>` 5本 src 充足）。**サブエージェントレビュー PASS（ブロッカーなし・Low 3件: コメント文言は反映済み、`_find_dep_by_fn` の同一性比較の将来脆さ・温存デッドコードの lint 警告は許容）**。**教訓**: Colab/tunnel 専用の姑息療法（非同期ジョブ・keep-alive）はローカル実行で UX を損なう。実行環境に応じ同期/非同期を切替可能にしておく。詳細は ERROR_LOG ERR064。 |
| **RouteA 動画: prompt クリア/削除が UI に反映されない不具合を Haystack 流の機能分割で根治＋Colab 姑息療法（Layer A/B）撤去（ERR063・2026-06-26）** | **✅ 完了（Playwright 実機検証 PASS）**。依頼（逐語要旨）: 「3 あと prompt クリアが効いていない もしくはUIに反映されていない ので対処 haystackパイプライン haystackコンポーネント 疎結合 i/o契約 機能分割を踏まえ `.github\copilot-instructions.md` これも踏まえ改修おねがい」。途中判断: Layer D は機能温存しコメントのみ修正、Layer A は RouteA/BEN2 限定の差し戻し（共有プリミティブは温存）。**真因**: overlay 描画ハンドラ群（`clear_prompt`/`select_sam2_prompt`/`extend_box_to_edge`/`detect_text_boxes_for_video`/`apply_selected_boxes`/`remove_selected_prompt_points`/`remove_selected_prompt_boxes`）が描画基準に **overlay 焼き込み済みの `prompt_canvas`** を渡しており、`draw_prompt_overlay`（`input_image.copy()` 上に state 再描画の純関数）が「前回 overlay 入り画像」の上へ新 state を重ねるため、state を空にしても古い overlay が視覚的に残った。**対処（state を真実の源にする機能分割）**: ①クリーンフレーム専用 `prompt_base_image = gr.State(...)` を追加し、全 overlay 描画は常にこのクリーンベースから行う。②フレーム取得系（`extract_first_frame_outputs` を 5 戻り値化＋新規 `extract_first_frame_with_base`/`extract_prompt_frame_with_base`）が canvas と base へ同一クリーンフレームを返し、base 更新は「フレームが変わるイベントのみ」に限定。detect/apply/削除は base を出力に含めない（フレーム不変・追加情報は state が保持）。③各ハンドラの描画基準入力を `prompt_canvas`→`prompt_base_image` に再配線。**姑息療法撤去（Windows local 直結化に伴い不要）**: Layer B＝`prewarm_ben2_models()` と `__main__` 呼び出し・未使用 import `warm_up_ben2_in_pipelines` 削除。Layer A＝`ben2_components` の BEN2 warm_up を `run_with_progress_keepalive` ラップ→直呼び＋`_notify_progress` に戻し未使用 import を整理。Layer C（非同期ジョブ `JobManager`+`gr.Timer`）・Layer D（`release_text_detector` の VRAM 解放、docstring から Colab 記述除去のみ）は温存。共有プリミティブ `run_with_progress_keepalive`/`warm_up_ben2_in_pipelines` は本番呼出元が消えたが Colab 再運用想定で docstring に温存理由明記し残置。**検証（ERR035 準拠）**: RED→GREEN ソース配線回帰 `tests/unit/test_routea_movie_prompt_clear_wiring.py`（5件）＋ `test_ben2_components.py` を keep-alive 非経由検証へ更新。**Playwright 実機検証 PASS**＝`outputs/verify_routea_prompt_clear.py`（動画ロード→box 描画→クリアの canvas 差分: `diff(A,B)=1.523`/`diff(B,C)=1.523`/`diff(A,C)=0.000`＝クリア後クリーンフレームに完全一致）。**非 integration 289 passed / 3 deselected**、RouteA app `--help` 正常、get_errors=0（既存の `BEN2RouteAVideoExtractor` 未使用 lint は本変更外）。サブエージェントレビュー: Blocker/Major なし、Minor（温存プリミティブの docstring 明記・ERR035 実機検証）は本対応で解消。**環境メモ**: 実機検証のため venv に playwright 1.60.0+chromium を導入（`uv pip install -p .venv\Scripts\python.exe playwright`）。詳細は ERROR_LOG ERR063。 |
| **Windows 実行中に `ProactorBasePipeTransport._call_connection_lost` が `WinError 10054` を連発するノイズを抑制（ERR062・2026-06-26）** | **✅ 完了**。依頼（逐語要旨）: 「`.venv\Scripts\python.exe gradio_app_sam2_ben2_route_a_for_Movie.py` 実行中に `ConnectionResetError: [WinError 10054]` が出る。エラー対応お願いします。対応フローは `.github\copilot-instructions.md`」。**真因**: Windows 既定の asyncio Proactor ループで、ブラウザ切断/再接続時に `connection_lost` callback が noisy に例外ログを吐く既知パターン。RouteA は Timer ポーリングで接続張替えが起こりやすく、当たりやすい。**第1案が不発**: `gradio` import 前に `asyncio.WindowsSelectorEventLoopPolicy()` を設定したが、ユーザー実機ログで**エラーが継続**（トレースバックが `proactor_events.py` のまま）。Gradio が内部で使う uvicorn は Windows で自前の Proactor ループを使い続けるため、グローバル policy 切替がサーバーループに反映されない。**確定対処**: `gradio` import 前に Windows のみ、`asyncio.proactor_events._ProactorBasePipeTransport._call_connection_lost` を `functools.wraps` でラップし、**`ConnectionResetError`（WinError 10054）だけ**を握って `return None`（他例外は素通し＝全握り潰しではない）。旧 Selector policy は撤去。`import asyncio`（未使用化）も削除。**検証**: `gradio_app_sam2_ben2_route_a_for_Movie.py --help` 正常、get_errors=0（既存の `BEN2RouteAVideoExtractor` 未使用 lint は本変更外）。**注意**: ログノイズ抑制が主眼で、実ネットワーク断そのものは別問題。**ERR035 留意**: 実機 Playwright での連発消失確認はユーザー要確認。詳細は ERROR_LOG ERR062。 |
...
| **最終更新日** | 2026-07-01（動画1回の実行が2タイムスタンプフォルダに分裂し overlay 専用フォルダに成果物が無い不具合を共有コンポーネントで根治〔ERR069〕。`VideoReader` が run_timestamp を metadata へ1回だけ発行し、`resolve_run_timestamp` で全 extractor＋`TrackingOverlayWriter` が同一 `outputs/<ts>/` を共有。overlay は `output_mode` 準拠〔video/both→webm、sequence/both→PNG〕に、連番出力は rgba/alpha のみ〔preview 除外〕に統一。全5動画アプリの tracking_overlay 実行入力へ output_mode を配線。test_tracking_overlay に5テスト追加＝非 integration 392 passed / 3 deselected、5アプリ --help OK、get_errors=0、サブエージェントレビュー指摘0。後方互換〔metadata 欠落時は now() fallback〕。`segment-anything`/`samurai` 無改変。リポジトリは `I:\AI_picasso\Matting-Anything` 運用。） |
---
