...
| **最終更新日** | 2026-07-01（DEVA 方式 2 動画アプリ〔手動 7864 / ハイブリッド 7865〕へ「検出起点フレーム」機能を追加。共有 `DevaSemiOnlineTracker.run()` に後方互換の `detection_start_frame` を追加し、`detection_start_frame>0` のとき clip0 を起点フレームで seed ＋ `[0, start)` を双方向 backward pass でカバー〔SAMURAI=forward-only は raise ガード〕。両 UI に検出起点フレーム Accordion＋スライダ＋起点表示、`_effective_read_frames` で読み取り窓拡張、status に `start_frame=` 表示。tracker テスト 6 件追加＝25 passed、wiring 2 件更新、非 integration 382 passed / 3 deselected、両 `--help` OK、get_errors=0。サブエージェントレビュー APPROVE〔LOW 2 反映〕。**Playwright 実機検証 PASS**＝`outputs/verify_manual_deva_start_frame.py`〔7864・box seed・start_frame=1・frames=3・video 5 本 readyState=4〕/`outputs/verify_hybrid_deva_start_frame.py`〔7865・text=person・start_frame=1・frames=3・video 5 本 readyState=4〕＝ERR035 充足で**両 DEVA アプリ UI を fixed 確定**。後方互換〔start=0 で従来一致〕・基底/propagator/`segment-anything`/`samurai` 無改変。リポジトリは `I:\AI_picasso\Matting-Anything` 運用。） |
---
