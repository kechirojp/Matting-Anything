...
| **最終更新日** | 2026-06-23（RouteA に SAM2 マスクを α の床として加算合成する mask_floor_mode[none/screen/lighten=比較明] を新設。難易度の高い動画[前景ブラー・背景同系色・高速スティック]のちらつきを SAM2 安定マスクで底上げ補完。既定 none で従来挙動不変。route_a_common 純関数+config+ben2 両経路+Gradio UI 配線、test 23 passed・wiring 4 passed・--help 正常・get_errors=0・レビュー PASS。実機効果はユーザー要確認。） |
---
