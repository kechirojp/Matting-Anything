...
def prewarm_ben2_models() -> int:
    """``demo.launch()`` の前に BEN2 重みを事前ロードして HF キャッシュへ取得する。

    BEN2 の約380MB DL をリクエスト処理中（SSE ストリーム中）に走らせると、HF 未認証リクエストの
    レート制限で DL が長時間化し Colab/gradio.live の SSE が idle 切断され全出力「Error」になる
    （ERR057＝ERR055/ERR056 follow-up）。SSE 接続が無い起動前に重みを取得しておけば、DL が遅くても
    起動セル内で完結し、リクエスト時はキャッシュ済み重みを即時ロードできる。両タブ（追跡あり/なし）の
    実際の Pipeline インスタンスを warm_up するため、リクエスト時の再 DL は発生しない。GPU/ben2 が
    無い環境で失敗しても起動は止めない（リクエスト時の keep-alive フォールバックがある）。
    """
    return warm_up_ben2_in_pipelines([get_route_a_pipeline(), get_route_a_only_pipeline()])
...
"""
    ...
    run_btn.click(
        ...
        outputs=[route_a_job_id, run_status, route_a_timer, run_btn],
    )
    route_a_timer.tick(
        poll_route_a_job,
        inputs=[route_a_job_id],
        outputs=[rgba_video, alpha_video, preview_video, tracking_overlay_video, sequence_files, sequence_dirs, run_status, route_a_timer, run_btn],
    )
    ...
    route_a_only_run_btn.click(
        ...
        outputs=[route_a_only_job_id, route_a_only_run_status, route_a_only_timer, route_a_only_run_btn],
    )
    route_a_only_timer.tick(
        poll_route_a_only_job,
        inputs=[route_a_only_job_id],
        outputs=[route_a_only_rgba_video, route_a_only_alpha_video, route_a_only_preview_video, route_a_only_sequence_files, route_a_only_sequence_dirs, route_a_only_run_status, route_a_only_timer, route_a_only_run_btn],
    )
...
if __name__ == "__main__":
    args = parse_args()
    # SSE 接続が無い起動前に BEN2 重みを事前ロードし、リクエスト中の長時間 DL による
    # gradio.live の idle 切断（全出力「Error」）を防ぐ（ERR057）。
    prewarm_ben2_models()
    demo.queue()
