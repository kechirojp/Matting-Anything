...
def run_video_background_removal(
    ...
    try:
        ...
        if not points and box is None:
            raise gr.Error("第 1 フレーム上で point または box prompt を指定してください。")
        output_mode = normalize_output_mode(output_mode_label)
        progress(0.05, desc="Pipeline を起動しています")
        result = get_video_pipeline().run(
            {
                "video_reader": {"video_path": video_path, "max_frames": int(max_frames), "frame_step": int(frame_step)},
                "sam2_video_propagator": {"points": points, "labels": labels, "box": box},
                ...
                "video_writer": {"rgba_codec": rgba_codec},
            },
            include_outputs_from={"video_reader", "sam2_video_propagator", "transparent_bg_video", "video_writer", "frame_sequence_writer"},
        )
        progress(0.95, desc="出力を整理しています")
        base = result.get("transparent_bg_video", {}).get("matte", {})
...
with gr.Blocks(title="SAM2 + Transparent Background Haystack for Movie") as demo:
    ...
    gr.Markdown(
        """
動画をアップロードすると第 1 フレームを自動表示します。最短フローは
`Input Video` → 第 1 フレーム上で bbox を2クリック → `動画背景除去を実行` です。
詳細設定は必要な場合だけ開いてください。
"""
    )
    ...
    with gr.Accordion("Advanced: 動画処理設定", open=False):
        with gr.Row():
            max_frames = gr.Slider(1, 2000, value=300, step=1, label="最大処理フレーム数")
